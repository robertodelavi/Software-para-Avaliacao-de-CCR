<?php
	...
	if ($error_cpf == 1){
		...
		if (isset($_POST['phone2']) && $_POST['phone2'] != ""){
			...
			if (mysql_num_rows($result) == 0){
				$phone2 = $_POST['phone2'];
				mysql_query("INSERT INTO phone (number, patient, professional) VALUES ('$phone2',NULL, '$cpf')") or die ("Erro ao inserir o número de telefone");
			}
		}
		$result = $result = mysql_query('SELECT number FROM phone WHERE number = "'.$_POST['phone'].'"');
		if (mysql_num_rows($result) == 0){
			mysql_query("INSERT INTO phone (number, patient, professional) VALUES ('$phone',NULL,'$cpf')") or die ("Não foi possível inserir o número de telefone");
		}
		$_SESSION['name'] = "";
		$_SESSION['last_name'] = "";
		$_SESSION['email'] = "";
