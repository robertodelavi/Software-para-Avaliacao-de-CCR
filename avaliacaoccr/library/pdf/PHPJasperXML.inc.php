...
public function variable_calculation($m=0,$ini,$end)
{
	foreach($this->arrayVariable as $k=>$out)
	{
		switch($out[calculation])
		{
			...
			for($i=$ini;$i<$end;$i++)
			{
				$sum += $this->arraysqltable[$i]["$out[target]"];
				$table["$out[target]"];
			}
			}
...
{	$gname=$this->arrayband[0][gname]."";
	...
	{
	if($headerY == $this->arrayPageSetting[topMargin]+$this->arraypageHeader[0][height]){
	$footerY = $headerY + 15;
	}else{
		$footerY = $headerY - 40;
	}
	$this->arraygroup[$gname][groupFooter][0][y_axis]=$this->arraydetail[0][y_axis]+$footerY;
	}
...
{	$limit = $this->arrayPageSetting[pageHeight]-($this->arraypageHeader[0][height]+$this->arraypageFooter[0][height]+$this->arrayPageSetting[topMargin]+$this->arrayPageSetting[bottomMargin]+10).'<br>';
