<?php
	$sql = "SELECT *
			FROM cargo
			WHERE car_codigo = ".$_GET['id'];
	$cargo = $data->find('dynamic',$sql);
?>

<div id="table">
	<h2>Edição do Cargo</h2>
    <form action="?module=cadastros&acao=update_cargo" id="frmCadastro" method="post">
        
        <input type="hidden" name="car_codigo" value="<?php echo $cargo[0]['car_codigo']; ?>"  />        
        ...
        <div class="linha">
            ...
            <div class="coluna" style="margin-right: 23px; margin-left:0px;">
            	<input name="car_descricao" value="<?php echo $cargo[0]['car_descricao']; ?>" id="car_descricao" type="text" size="61" style="text-transform: uppercase;" />
            </div>
            <div style="clear: both;"></div>           
		</div>        
